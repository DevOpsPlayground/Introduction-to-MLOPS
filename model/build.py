...
def get_approved_package(package_group_name):
    """ Get the latest approved model package for a model package group.
    Args:
...
def get_job_id(env, pipeline_name):
    """ Get the latest ExecutionID based on the Codepipeline Stage.
    Args:
        env: (str) The current Deployment Stage.
        pipeline_name: (str) CodePipeline name.

    Returns: CodePipeline Execition ID.
    """
    try:
...
def extend_dev_params(args, stage_config):
    """ Extend the development stage configuration to the pipeline execution.

    Args:
        args: (parser) Parsed known arguments.
        stage_config: (dict) Current configuration for the stage.

    Returns: A dictionary of configured CloudFormation parmaters for the stage.
    """
...
def extend_prd_params(args, stage_config):
    """ Extend the production stage configuration to the pipeline execution.
